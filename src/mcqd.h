...
#endif
  Maxclique(const bool* const*, const int, void (*)(int*, int)=nullptr, const float=0.025);
...
Maxclique::Maxclique (const bool* const* conn, const int sz, void(*callback_fun)(int*, int), const float tt) : pk(0), callback(callback_fun), level(1), Tlimit(tt), V(sz), Q(sz), QMAX(sz) {
  assert(conn!=0 && sz>0);
...
void Maxclique::expand(Vertices R) {
  while (R.size()) {
	...
	QMAX = Q;
        if(callback!=nullptr) callback(QMAX.i, QMAX.size());
      }    
      Rp.dispose();
      Q.pop();
    }
